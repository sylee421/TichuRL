...
class DQN(object):
    ...
    def build_network(self, name):
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            output = tf.layers.dense(h3, self.action_size)
    ...
    def train_network(self):
        ...
        q_value_tmp = self.sess.run(self.prediction_Q, feed_dict={self.states:next_states})      
        
        target = []
        for i in range(self.replay.batch_size):
            legal_q_value = q_value_tmp[i] * leg_act_onehots[i]
            if np.size(legal_q_value[legal_q_value!=0]) != 0:
                max_q = np.max(legal_q_value[legal_q_value!=0])
            else:
                max_q = np.max(legal_q_value)
            max_idx = np.where(legal_q_value==max_q)[0][0]

            if terminals[i]:
                target.append(rewards[i])
            else:
                target.append(rewards[i] + self.discount_factor * target_Q[i][max_idx])

        self.loss, _  = self.sess.run(self.train_op, feed_dict={self.states: states, self.actions: actions, self.target: target})
        pass
...
class Agent_2DQN(object):
    def __init__(self, sess, is_training=True):
        ...
        self.epsilon_decay_steps = 10000 # epsilon이 0.1이 되기까지의 time step 수
    ...
    def DQN_input(self, state, next_state, reward, terminal, action):
        ...
        leg_act_onehot = self.legal_act_onehot(next_state)
    ...
    def train_d(self, state, next_state, reward, terminal, action):
        state_in, next_state_in, action_in, leg_act_onehot = self.DQN_input(state, next_state, reward, terminal, action)
        ...
        if terminal:
            ...
            if self.episode % 100 == 0:
                print('episode: ', self.episode, 'score: ', int(self.score), 'loss: ' , self.mean_loss/10, 'cur loss : ', self.dqn.loss, 'epsilon ', self.eps)
                self.mean_loss = 0
            elif self.episode % 100 >= 90 :
                self.mean_loss += self.dqn.loss
            self.score = 0

    # 학습한 weights를 save하고 load함
    def save(self):
        checkpoint_dir = 'dqn5_tensorflow_p'
