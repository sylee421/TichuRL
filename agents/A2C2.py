...
# A2C adaptng, actor, critic network works seperately
...
class A2C(object):
    ...
    def __init__(self, state_size, action_size, value_size, sess, learning_rate, replay, discount_factor):
        ...
        self.prediction_P = self.build_network('pred')
        self.prediction_v = self.build_value_network('value')
        self.target_v = self.build_value_network('target')
    ...
    def build_network(self, name):
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            policy = tf.layers.dense(h3, self.action_size, activation=tf.nn.softmax)

            return policy
            pass

    def build_value_network(self, name):
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            h1 = tf.layers.dense(self.states, 125, activation=tf.nn.relu)
            h2 = tf.layers.dense(h1, 125, activation=tf.nn.relu)
            value = tf.layers.dense(h2, self.value_size)
    ...
    def build_optimizer(self):
        ...
        leg_prediction_P = self.prediction_P * self.leg_act_onehots

        log_policy = tf.log(tf.clip_by_value(self.prediction_P, 1e-20, 1.0))
        log_prob = tf.reduce_sum(log_policy * actions_one_hot, axis=1)

        value_loss = tf.reduce_mean(tf.square(tf.subtract(self.target, self.prediction_v)))

        policy_loss = tf.reduce_mean(log_prob * tf.subtract(self.target, self.prediction_v))

        train_op1 = tf.train.AdamOptimizer(self.lr).minimize(policy_loss)
        train_op2 = tf.train.AdamOptimizer(self.lr).minimize(value_loss)

        return policy_loss, value_loss, train_op1, train_op2, self.prediction_P, leg_prediction_P, log_policy, log_prob
        pass
    ...
    def train_network(self):
        ...
        target_V = self.sess.run(self.target_v, feed_dict={self.states: next_states})
        ...
        self.policy_loss, self.value_loss, _, _, prediction_P, leg_prediction_P, log_policy, log_prob = self.sess.run(self.train_op, feed_dict={self.states: states, self.actions: actions, self.target: target, self.leg_act_onehots: leg_act_onehots})
    ...
    def update_target_network(self):
        copy_op = []
        pred_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='value')
    ...
    def train_d(self, state, next_state, reward, terminal, action):
        ...
        if terminal:
            ...
            if self.episode % 100 == 0:
                print('episode: ', self.episode, 'score: ', int(self.score), 'loss: ' , self.mean_loss/10, self.mean_loss2/10, 'cur loss : ', self.a2c.policy_loss, self.a2c.value_loss, 'epsilon ', self.eps)
                self.mean_loss = 0
                self.mean_loss2 = 0
            elif self.episode % 100 >= 90 :
                self.mean_loss += self.a2c.policy_loss
                self.mean_loss2 += self.a2c.value_loss
            self.score = 0
